...
public class GenieState_DroneSwingWave : GenieStateBase
{
    ...
    public float groundCutTimingTime = 3f;
    public float groundCutTime = 3f;
    public float groundCutMoveSpeed = 1f;
...
#region GroundCut
    ...
    public void GroundCut(float t)
    {
        ...
        for(int i = 0; i < count; ++i)
        {
            ...
            GetGridLine(ref _fallLine,right,groundCutThickness);
            foreach(var item in _fallLine)
            {
                item.SetMove(false,((float)i) * groundCutTimingTime + Random.Range(0f,0.2f),groundCutMoveSpeed,groundCutTime);
            }
...
#region DroneStuff
    ...
    public void SpawnCoreDrone(float time)
    {
        ...
        _timeCounter.SkipSequencer("GroundCut",groundCutPatternTime - 1f);
    }
