...
[System.Serializable]
public class PathManagerEx : MonoBehaviour
{
    [System.Serializable]
    public class PathClass
    {
        ...
        public List<MovePointEx> movePoints = new List<MovePointEx>();
        ...
        public MovePointEx GetNextPoint(ref int point, out bool isEnd)
        {
        ...
        public MovePointEx FindNearestPoint(Vector3 position, out int target)
        {
        ...
        public MovePointEx GetPoint(int point)
        {
            return movePoints[point];
        }
    ...
    public MovePointEx GetNextPoint(string path, ref int currentPoint)
    {
        currentPoint = currentPoint >= FindPath(path).movePoints.Count - 1 ? 0 : currentPoint + 1;
        return GetPoint(path, currentPoint);
    }

    public MovePointEx GetPoint(string path, int point)
    {
        return FindPath(path).movePoints[point];
    }


    public List<MovePointEx> GetList(string path) 
    {
        var target = FindPath(path);
        if(target == null)
            return null;
    ...
    public void AddPoint(string path, MovePointEx point)
    {
    ...
    public void DeletePoint(string path, MovePointEx point)
    {
