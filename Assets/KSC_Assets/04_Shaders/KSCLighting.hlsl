...
{
    float3 halfVec = SafeNormalize(float3(lightDir) + float3(viewDir)); //하프 벡터 구하기  
    half NdotH = saturate(dot(normal, halfVec)); // 블린 퐁 계산
    half modifier = pow(NdotH, smoothness); // 반사율 조정 
...
{
    half NdotL = saturate(dot(normalWS, lightDirectionWS)*0.5+0.5); // 하프렘버트
